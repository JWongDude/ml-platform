...
def initializeInference():
  if inference_parameters.allInferenceInputsRecieved():
    # try: 
      ...
      """ 
      # Placeholder Logic:
      # Loading code, keep image path list in controller and pass reference to the model
      uploaded_path = 'sandbox/Test_Images'  
      inference_parameters.image_directory = [str(image_path) for image_path in Path(uploaded_path).iterdir()]        

      # Slider min / max code
      slider_max = inference_parameters.getImageDirectoryLength()

      # results = inference_parameters.module.prediction(index)
      # Classifications, output of model logic. Save output in controller
      inference_parameters.results = ["Inclusion", "Inclusion", "Rolled", "Rolled", "Scratches", "Scratches"]

      # Pass index=0 to inference view
      image_path = inference_parameters.image_directory[0]
      label = inference_parameters.results[0]
